...
$release = Join-Path $root "build\v1.0"
...
try {
    ...
    & $pwsh -NoProfile -File (Join-Path $root "tools\build-weeko-v10-debug.ps1") -BaseApktoolProject $BaseApktoolProject -AndroidSdkPath $AndroidSdkPath -JdkPath $jdk -ApktoolJarPath $ApktoolJarPath *> (Join-Path $logs "debug-candidate-build.log")
    if ($LASTEXITCODE -ne 0) { throw "The v1.0.0 source and patch build failed; see release-logs/debug-candidate-build.log." }

    $unsigned = Join-Path $release "helper-v1-unsigned.apk"
    $aligned = Join-Path $release "Weeko-v1.0.0-release-aligned.apk"
    $final = Join-Path $release "Weeko-v1.0.0-release.apk"
    if (!(Test-Path -LiteralPath $unsigned)) { throw "The v1.0.0 unsigned candidate was not produced." }
    ...
    if ($certificateSha256 -ne "E654E9A275921CB213CD0AE3EB5D8C35FD5DDE1C84E32F71D3B6D42EED328CD5") { throw "Final APK certificate does not match the new permanent Weeko v3 release certificate." }
    ...
    if ($badging[0] -notmatch "^package: name='io.github.mxwf.weeko' versionCode='12' versionName='1.0.0'") { throw "Final APK package or version does not match v1.0.0." }
    $hash = (Get-FileHash -LiteralPath $final -Algorithm SHA256).Hash.ToUpperInvariant()
    $size = (Get-Item -LiteralPath $final).Length
    Set-Content -LiteralPath (Join-Path $release "v1-release-summary.txt") -Value @(
        "path=$final", "sha256=$hash", "size=$size", "package=io.github.mxwf.weeko",
        "versionCode=12", "versionName=1.0.0", "signingKey=$KeyAlias",
