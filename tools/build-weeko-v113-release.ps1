...
$source = Join-Path $root "tools\build-weeko-v112-release.ps1"
$text = [IO.File]::ReadAllText($source)
$text = $text.Replace("build\v1.1.2", "build\v1.1.3")
$text = $text.Replace("build-weeko-v112-debug.ps1", "build-weeko-v113-debug.ps1")
$text = $text.Replace("Weeko-v1.1.2", "Weeko-v1.1.3")
$text = $text.Replace("v1.1.2-release-summary.txt", "v1.1.3-release-summary.txt")
$text = $text.Replace("versionCode='16'", "versionCode='17'")
$text = $text.Replace("versionCode=16", "versionCode=17")
$text = $text.Replace("1.1.2", "1.1.3")
$generated = Join-Path $root "tools\.build-weeko-v113-release-generated.ps1"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Weeko v1.1.3 release build failed." }
} finally {
