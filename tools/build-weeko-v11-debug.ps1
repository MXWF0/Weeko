...
$source = Join-Path $root "tools\build-weeko-v101-debug.ps1"
$text = [IO.File]::ReadAllText($source)
$text = $text.Replace("build\v1.0.1", "build\v1.1.0")
$text = $text.Replace("replay-weeko-v101-patches.ps1", "replay-weeko-v11-patches.ps1")
$text = $text.Replace("versionCode=13", "versionCode=14")
$text = $text.Replace("versionCode: 13", "versionCode: 14")
$text = $text.Replace("versionCode=''13''", "versionCode=''14''")
$text = $text.Replace("1.0.1", "1.1.0")
$text = $text.Replace("v101", "v11")
$generated = Join-Path $root "tools\.build-weeko-v11-debug-generated.ps1"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Weeko v1.1.0 debug build failed." }
} finally {
