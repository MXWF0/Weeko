...
$source = Join-Path $root "tools\build-weeko-v101-release.ps1"
$text = [IO.File]::ReadAllText($source)
$text = $text.Replace("build\v1.0.1", "build\v1.1.0")
$text = $text.Replace("build-weeko-v101-debug.ps1", "build-weeko-v11-debug.ps1")
$text = $text.Replace("Weeko-v1.0.1", "Weeko-v1.1.0")
$text = $text.Replace("v1.0.1-release-summary.txt", "v1.1.0-release-summary.txt")
$text = $text.Replace("versionCode='13'", "versionCode='14'")
$text = $text.Replace("versionCode=13", "versionCode=14")
$text = $text.Replace("1.0.1", "1.1.0")
$generated = Join-Path $root "tools\.build-weeko-v11-release-generated.ps1"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Weeko v1.1.0 release build failed." }
} finally {
