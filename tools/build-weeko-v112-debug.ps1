...
$text = $text.Replace("replay-weeko-v11-patches.ps1", "replay-weeko-v112-patches.ps1")
$text = $text.Replace("versionCode=14", "versionCode=16")
$text = $text.Replace("versionCode: 14", "versionCode: 16")
$text = $text.Replace("versionCode=''14''", "versionCode=''16''")
$text = $text.Replace("1.1.0", "1.1.2")
$text = $text.Replace("v1.1.0", "v1.1.2")
$generated = Join-Path $root "tools\.build-weeko-v112-debug-generated.ps1"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Weeko v1.1.2 debug build failed." }
} finally {
