...
$project = Join-Path $release "repro-apktool-fluent-stage5"
$sourceDex = Join-Path $root "build\v0.6\source-dex\classes.dex"
$patchScript = Join-Path $root "tools\replay-weeko-v08-fluent-stage5-patches.ps1"
$unsigned = Join-Path $release "helper-fluent-stage5-unsigned.apk"
$aligned = Join-Path $release "Weeko-v0.7.0-fluent-stage5-debug-aligned.apk"
$final = Join-Path $release "Weeko-v0.7.0-fluent-stage5-debug.apk"
...
foreach ($required in @($base, $sourceDex, $patchScript, $java, $apktool,
    ...
    if (!(Test-Path -LiteralPath $required)) { throw "Required Fluent Stage 5 build input is missing: $required" }
}
...
$replayLog = Join-Path $release "fluent-stage5-replay.log"
& $patchScript -ProjectPath $project *> $replayLog
if (!$?) { throw "Fluent Stage 5 patch replay failed; see $replayLog." }

if (Test-Path -LiteralPath $unsigned) { Remove-Item -LiteralPath $unsigned -Force }
& $java -jar $apktool b $project -o $unsigned -f *> (Join-Path $release "fluent-stage5-apktool-build.log")
...
Set-Content -LiteralPath (Join-Path $release "fluent-stage5-source-dex-injection.txt") -Value "Injected $sourceDex as $newName" -Encoding UTF8

if (Test-Path -LiteralPath $aligned) { Remove-Item -LiteralPath $aligned -Force }
& (Join-Path $buildTools "zipalign.exe") -f -p 4 $unsigned $aligned *> (Join-Path $release "fluent-stage5-zipalign.log")
if ($LASTEXITCODE -ne 0) { throw "Zip alignment failed." }
if (Test-Path -LiteralPath $final) { Remove-Item -LiteralPath $final -Force }
$env:JAVA_HOME = $jdk
& (Join-Path $buildTools "apksigner.bat") sign --ks $debugKeystore --ks-key-alias androiddebugkey --ks-pass pass:android --key-pass pass:android --out $final $aligned *> (Join-Path $release "fluent-stage5-apksigner-sign.log")
if ($LASTEXITCODE -ne 0) { throw "Debug signing failed." }
& (Join-Path $buildTools "zipalign.exe") -c -P 4 -v 4 $final *> (Join-Path $release "fluent-stage5-zipalign-verify.log")
...
$signatureOutput | Set-Content -LiteralPath (Join-Path $release "fluent-stage5-apksigner-verify.log") -Encoding UTF8
...
$badging | Set-Content -LiteralPath (Join-Path $release "fluent-stage5-apk-badging.txt") -Encoding UTF8
if ($badging[0] -notmatch "^package: name='io.github.mxwf.weeko' versionCode='7' versionName='0.7.0-fluent-stage5'") { throw "APK metadata does not match Fluent Stage 5." }

$hash = (Get-FileHash -LiteralPath $final -Algorithm SHA256).Hash.ToUpperInvariant()
$size = (Get-Item -LiteralPath $final).Length
Set-Content -LiteralPath (Join-Path $release "fluent-stage5-release-summary.txt") -Value @(
    "path=$final", "sha256=$hash", "size=$size", "package=io.github.mxwf.weeko",
    "versionCode=7", "versionName=0.7.0-fluent-stage5", "stage=fluent-stage5", "signing=androiddebug-debug"
) -Encoding UTF8
